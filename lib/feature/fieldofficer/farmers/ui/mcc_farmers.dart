...
import '../../../farmers/presentation/blocs/farmers_cubit.dart';
import '../../../farmers/presentation/pages/update_farmer.dart';

class MccFarmers extends StatefulWidget {
  final PickupLocationEntityModel mccModel;
  const MccFarmers({super.key, required this.mccModel});

  @override
  State<MccFarmers> createState() => _MccFarmersState();
}

class _MccFarmersState extends State<MccFarmers> {
  PickupLocationEntityModel mccModel = PickupLocationEntityModel();
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => sl<FarmersCubit>()..getMccFarmers(mccModel.id!),
                  ...
                  searching
                      ? SizedBox(
                        ...
                        child: searchField(searchCont, (query) {
                          context.read<FarmersCubit>().searchMccFarmer(query);
                        }),
                      )
                      : Text("${mccModel.name} Farmers"),
              actions: [
                IconButton(
                  ...
                  onPressed: () {
                    setState(() {
                      if (searching) {
                        searchCont.clear();
                        context.read<FarmersCubit>().searchMccFarmer('');
                      }
                      searching = !searching;
                    });
                  },
    ...
    return BlocConsumer<FarmersCubit, FarmersState>(
      ...
      builder: (context, state) {
        if (state.uiState == UIState.initial) {
          context.read<FarmersCubit>().getMccFarmers(mccId);
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    const SizedBox(width: 5),
                                    Text(
                                      'Route. ${state.filteredFarmers![index].routeName ?? 'N/A'}',
                                      style: const TextStyle(fontSize: 12),
                                    ),
                                  ],
                                ),
